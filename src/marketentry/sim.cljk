(ns marketentry.sim
  ...
  commit), then shows HARD-hold scenarios (fabricated jurisdiction,
  QIP double-election, QIP tier-duration mismatch, engagement-fee
  mismatch, patent-tax unverified, double-draft, double-submit)."
...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (KHM, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up QIP double-election) ==")
    ...
    (println "== filing/submit eng-4 (QIP double-election -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up QIP tier-duration mismatch) ==")
    ...
    (println "== filing/submit eng-5 (QIP tier-duration mismatch -> HARD hold) ==")
    (println (exec-op actor "t11" {:op :filing/submit :subject "eng-5"} operator))

    (println "== jurisdiction/assess eng-6 (sets up patent-tax-unverified) ==")
    ...
    (println "== filing/submit eng-6 (patent-tax-unverified -> HARD hold) ==")

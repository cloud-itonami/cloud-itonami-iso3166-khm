  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Angkor Bridge Systems"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Angkor Bridge Systems" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest qip-double-election-is-held-and-unoverridable
  (testing "QIP double-election (both Option 1 AND Option 2 claimed) -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:qip-incentive-election-mismatch} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest qip-tier-duration-mismatch-is-held-and-unoverridable
  (testing "QIP claimed exemption-years not matching independently recomputed Group tier -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
          _ (assess! actor "t5bpre" "eng-5")
          _ (draft! actor "t5bpre" "eng-5")
          res (exec-op actor "t5b" {:op :filing/submit :subject "eng-5"} operator)]
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:qip-incentive-election-mismatch} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
...
(deftest patent-tax-unverified-is-held-and-unoverridable
  (testing "unverified Patent Tax certificate when required -> HARD hold"
    (let [[db actor] (fresh)
          _ (assess! actor "t7pre" "eng-6")
          _ (draft! actor "t7pre" "eng-6")
          res (exec-op actor "t7" {:op :filing/submit :subject "eng-6"} operator)]
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:patent-tax-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

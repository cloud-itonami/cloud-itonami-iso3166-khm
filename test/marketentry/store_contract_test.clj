...
(defn- exercise [s]
  (store/commit-record! s {:effect :engagement/upsert
                           :value {:id "eng-x" :operator "X Co" :jurisdiction "KHM"
  ...
  (store/commit-record! s {:effect :assessment/set
                           :path ["eng-x"]
                           :payload {:jurisdiction "KHM" :checklist ["a"] :spec-basis "x"}})
...
(deftest mem-and-datomic-parity
  (let [mem (store/seed-db)
    ...
    (is (= (:qip-group (:engagement m)) (:qip-group (:engagement d))))
    (is (= (:qip-claimed-exemption-years (:engagement m)) (:qip-claimed-exemption-years (:engagement d))))

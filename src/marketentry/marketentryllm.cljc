(ns marketentry.marketentryllm
  ...
  the KHM public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world QIP/portal submission."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けQIP/ポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "qip-group=" (:qip-group e)
                        " qip-claimed-exemption-years=" (:qip-claimed-exemption-years e)
                        " patent-tax-verified?=" (:patent-tax-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-patent-tax? e))
                              (:patent-tax-verified? e)))
                   0.9 0.3)}))
